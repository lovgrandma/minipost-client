...
export const updateHistory = function(type = "video") {
    const appendHistory = () => {
        ...
        checkLoggedInAppropriateStorage();
        try {
            if (window && cookies.get('loggedIn')) {
                if (window.location) {
...
// Creates storage for history and subscription data if data is corrupted or not accurate to user
const checkLoggedInAppropriateStorage = () => {
    let cookieCheck = JSON.parse(window.localStorage.getItem('mediahistory'));
    if (cookies.get('loggedIn')) {
        if (!cookieCheck || cookieCheck.user != cookies.get('loggedIn')) {
            window.localStorage.removeItem('mediahistory');
            let mediaObject = { user: cookies.get('loggedIn'), history: [], subscribed: [] };
            window.localStorage.setItem('mediahistory', JSON.stringify(mediaObject));
        }
    }
}
